...
void test(NodeNetwork *nodeNet, NodeConfiguration *nodeConfig, uint size, uint batch_size) {
    ...
    // for (j = 0; j < total; j++) {
    //     truncPr_1(c, a, m, size, ring_size, map, nodeNet); 
    //     truncPr_2_test(d, a, m, size, ring_size, map, nodeNet); 
    // }
    ...
    //         printf("a[%i]           : %llu\t", i, res3[i]);
    //         print_binary(res3[i], ring_size);

    //         printf("SQNN[%i]        : %llu\t", i, res[i]);
    //         print_binary(res[i], ring_size);
    //         printf("NewPrim[%i]     : %llu\t", i, res2[i]);
    //         print_binary(res2[i], ring_size);

    //         printf("expctd[%i]/2^m  : %llu\t", i, res_check[i]);
    ...
    // }
    // if (flag == 0) {
    //     printf(ANSI_COLOR_GREEN "TruncPR Correct!\n" ANSI_COLOR_RESET);
    // } else {
    //     printf(ANSI_COLOR_RED "TruncPR error!\n" ANSI_COLOR_RESET);
    // }
    ...
    gettimeofday(&start, NULL); //start timer here
    for (j = 0; j < total; j++) {
        // Rss_edaBit(c, d, size, ring_size, ring_size - 2, map, nodeNet);
    ...
    Rss_edaBit_trunc(c, d, e, size, ring_size, 5, map, nodeNet);
    Rss_Open(res, c, size, map, ring_size, nodeNet);
    Rss_Open_Bitwise(res2, d, size, map, ring_size, nodeNet);

    for (size_t i = 0; i < size; i++) {
        // printf("res[%i]: %llu\t", i, res[i]);
        // print_binary(res[i], ring_size);
    ...
    gettimeofday(&start, NULL); //start timer here
    for (j = 0; j < total; j++) {
        Rss_truncPr(c, a, m, size, ring_size, map, nodeNet);
        // truncPr_2_test(d, a, m, size, ring_size, map, nodeNet);
    }
